...
[NOTE]
In a disconnected environment, https://docs.openshift.com/container-platform/4.18/installing/installing_aws/installing-restricted-networks-aws-installer-provisioned.html#olm-restricted-networks-operatorhub_installing-restricted-networks-aws-installer-provisioned[you must disable the default `CatalogSources`].
...
DIGEST=$(oc image info -o json $HOSTNAME:8443/openshift/release-images:4.18.20-x86_64 | jq -r .contentDigest)
